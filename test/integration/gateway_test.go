...
func TestIntegration_NonStreaming_FullFlow(t *testing.T) {
	...
	if got := gw.registry.ModelTokens().WithLabelValues("granite4.2:8b", "prompt").Value(); got != 100 {
...
func TestIntegration_Streaming_FullFlow(t *testing.T) {
	...
	chunks := []bridge.ChatStreamChunk{
		{Model: "granite4.2:8b", Thinking: "considering options"},
		{Model: "granite4.2:8b", Message: bridge.Message{
			Role: "assistant",
			ToolCalls: []bridge.ToolCall{{
				Function: bridge.ToolCallFunction{
					Name:      "opencost.allocation",
		...
		{Model: "granite4.2:8b", Message: bridge.Message{Role: "assistant", Content: "you "}},
		{Model: "granite4.2:8b", Message: bridge.Message{Role: "assistant", Content: "spent $42"}},
		{
			Model:           "granite4.2:8b",
			Done:            true,
			DoneReason:      "stop",
