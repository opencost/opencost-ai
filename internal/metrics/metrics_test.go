...
func TestRegistry_ExposesDocumentedFamilies(t *testing.T) {
	...
	r.ModelTokens().WithLabelValues("granite4.2:8b", "prompt").Add(123)
	r.ModelTokens().WithLabelValues("granite4.2:8b", "completion").Add(45)
	...
	checks := []string{
		...
		`opencost_ai_gateway_model_tokens_total{kind="prompt",model="granite4.2:8b"} 123`,
